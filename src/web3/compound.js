...
const abi = [
  ...
  let cETH ="0x42a628e0c5f3767930097b34b08dcf77e78e4f2b";
  ...
  const troll = new web3.eth.Contract(trollAbi, comptroller);
    ...
    CEther.methods.mint().send({from: user, value: 1000000000000000000, gas : 2600000})
    ...
    troll.methods.enterMarkets([cETH, cDAI]).send({from: user  , gas : 2500000}).on('transactionHash', (hash) => {
        console.log("hash", hash)
    })
    .on('confirmation', (confirmationNumber, receipt) => {
        console.log("confirmationNumber", confirmationNumber)
...
//     CEther.methods.mint().send({from: user, value: 2000000000000000000, gas : 2600000})
//     .on('transactionHash', (hash) => {
//         console.log("hash", hash)
//     })
//     .on('confirmation', (confirmationNumber, receipt) => {
...
export const getBack = (user) => {
  ...
  let comptroller = "0x8d2a2836d44d6735a2f783e6418caedb86da58d8";
    ...
    CEther.methods.redeem(1).send({from: user, gas : 2600000})
    .on('transactionHash', (hash) => {
        console.log("hash", hash)
    })
    .on('confirmation', (confirmationNumber, receipt) => {
